...
func _init_db():
	...
	database.insert_row("multiple_choice", make_question("0", "ねこ", "猫", "犬", "狼", "描")) # TODO populate db, but only once. dont rely on unique failure. maybe there should be a set? one q for visually similar and one for not? would need to manually assign id and remove unique for question. maybe a speech to text where you ahve to say the kanji
	database.insert_row("multiple_choice", make_question("1", "いぬ", "犬", "天", "汰", "大"))
	database.insert_row("multiple_choice", make_question("2", "いちじ", "一時", "一語", "一間", "一円"))
	database.insert_row("multiple_choice", make_question("3", "いち", "一", "二", "三", "千"))
	database.insert_row("multiple_choice", make_question("", "", "", "", "", ""))


func make_question(id: String, question: String, answer: String, incorrect_0: String, incorrect_1: String, incorrect_2: String) -> Dictionary:
	var dict: Dictionary = {
		"id": id,
		"question": question,
